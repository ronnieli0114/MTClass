...
# data.dir <- file.path("/projects/compbio/users/yli459/gtex_eqtl")
# results.dir <- file.path(sprintf("/projects/compbio/users/yli459/gtex_eqtl/%s", suffix))

exp <- read_feather(file.path(data.dir, sprintf("tissue_level_expression/%s.ftr", tissue))) %>% as.data.frame()

Run.manova <- function(c) {
  ...
  for (gene in genes) {
    ...
    gene.res.list <- lapply(c(1:length(snps)), FUN=function(i) {
      ...
      skip_to_next <- FALSE
      manova.res <- tryCatch(manova(pheno.data ~ geno.data, na.action=na.exclude), error=function(e) {skip_to_next <<- TRUE})
      results <- tryCatch(summary(manova.res)$stats, error=function(e) {skip_to_next <<- TRUE})
      if (skip_to_next) {
        res[["gene"]] <- gene
        res[["variant"]] <- snp
        res[["pval"]] <- "NA"
...
# write to file
write.csv(final, file.path(results.dir, sprintf("%s_manova.txt", tissue)), quote=F, sep='\t', row.names=F, col.names=T)
message("=== done ===")
