### runs canonical correlation analysis
### and a statistical test of significance
### using the Wilks' Lambda statistic
...
RunCCA <- function(c) {
  ...
  for (g in genes) {
    ...
    gene.res.list <- mclapply(c(1:length(snps)), mc.cores=cores, FUN=function(i) {
      ...
      skip_to_next <- FALSE
      results <- tryCatch(cancor(x=pheno.data, y=geno.data), error=function(e) {skip_to_next <<- TRUE})
      if (skip_to_next) {
        res[["gene"]] <- g
        res[["variant"]] <- v
        res[["pval"]] <- "NA"
        ...
        p = dim(pheno.data)[2]
        q = dim(geno.data)[2]
        test.stat <- p.asym(rho, N, p, q, tstat='Wilks')
...
# write to file
write.table(final, file.path(results.dir, "brain_CCA.txt"), quote=F, sep='\t', row.names=F, col.names=T)
message("=== done ===")