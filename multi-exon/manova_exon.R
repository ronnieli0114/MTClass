...
for (tissue in brain_tissues) {
  ...
  for (c in chr.range) {
    ...
    for (gene_id in genes) {
      ...
      for (variant in variants) {
        ...
        skip_to_next <- FALSE
        dep_vars <- tryCatch(as.matrix(Xy[,1:dim(Xy)[2]-1]), error=function(e) {skip_to_next <<- TRUE})
        indep_vars <- tryCatch(Xy[,dim(Xy)[2]], error=function(e) {skip_to_next <<- TRUE})
  ...
  arrow::write_feather(final_df, file.path(final_path, "manova.ftr"))
